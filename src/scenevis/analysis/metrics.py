...
from scenevis.analysis.model import Metrics, Options, RegionStatistics


def statistics(values: NDArray[np.float32], *, epsilon: float) -> RegionStatistics:
...
def contrast_to_noise(
    target: RegionStatistics, local: RegionStatistics, *, robust: bool, epsilon: float
) -> float:
...
def signal_to_noise(stats: RegionStatistics, *, robust: bool, epsilon: float) -> float:
...
def primary_metrics(
    *,
    ...
    target: RegionStatistics,
    local: RegionStatistics,
    bright: RegionStatistics,
    options: Options,
) -> Metrics:
