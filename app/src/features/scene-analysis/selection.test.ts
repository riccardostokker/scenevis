...
import type { Polygon, Rectangle } from "./model";
...
describe("region suggestions", () => {
  it("surrounds a box target and retains the bright suggestion", () => {
    ...
    expect(regions.local_background.type).toBe("rectangle");
    if (regions.local_background.type !== "rectangle") throw new Error("expected rectangle");
  ...
  it("surrounds a lasso target", () => {
    ...
    const regions = suggestions(lasso, target);

    expect(regions.target).toBe(lasso);
    expect(regions.local_background.type).toBe("rectangle");
    if (regions.local_background.type !== "rectangle") throw new Error("expected rectangle");
    expect(regions.local_background.x).toBeLessThan(0.2);
    expect(regions.local_background.height).toBeGreaterThan(0.25);
  });
